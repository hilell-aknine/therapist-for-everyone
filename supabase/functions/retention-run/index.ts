...
מבנה ההודעה (4 חלקים, 75-115 מילים סה"כ):
1. פתיחה: שם פרטי + משפט שמשקף ב-1-2 קטעים שאתה זוכר אותו ספציפית. הזכר את ה-vision_1y שלו במילים קרובות לשלו (לא ציטוט מילולי שייראה מלאכותי). 1-2 משפטים.
2. גשר: ספר לו שהשיעור הבא בתור (שם השיעור) פותח בדיוק את החלק שהוא צריך עכשיו כדי להתקדם לשם. תהיה ספציפי על תוכן השיעור — מה הוא ילמד שמשרת ספציפית את החזון שלו. 2-3 משפטים.
3. CTA: הזמנה רכה ומעשית. שורה ריקה ואז הקישור על שורה משלו.
4. שורת opt-out: שורה ריקה אחת, ואז משפט קצר ומכבד שמציע אופציה לעצור — נסה ניסוחים כמו "אם זה לא הזמן — תוכל להגיב 'הסר' ואני אעצור." או וריאציה רכה דומה. חובה.
