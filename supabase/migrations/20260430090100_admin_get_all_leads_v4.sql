-- ============================================================================
-- Migration: admin_get_all_leads() v4 — surface utm_medium + utm_campaign
--
-- Bug fixed:
--   v3 returned only utm_source. The frontend `formatLeadSource()` helper
--   builds chip labels like "פייסבוק · paid_social" using utm_medium as the
--   suffix — but the column was missing from the RPC, so the medium was
--   always undefined and chips degraded to just "פייסבוק".
--
-- Changes vs v3 (20260428100000):
--   * Append utm_medium TEXT, utm_campaign TEXT to RETURNS TABLE.
--   * Source 1 (profiles): COALESCE(pq.utm_medium, p.utm_medium) etc.
--   * Source 2 (contact_requests): cr.utm_medium, cr.utm_campaign.
--   * No other logic change.
--
-- Note on landing_url/referrer:
--   Neither portal_questionnaires nor contact_requests has these columns.
--   They live on lead_attribution (last_landing_url, last_referrer_domain)
--   and are populated by the submit-lead Edge Function. The chip helper
--   reads them from `attrib`, not from `row` — no RPC change needed.
--
-- Schema reload at end to bypass PGRST schema-cache lag (same pattern as
-- 20260429120200_meta_spend_schema_reload.sql).
-- ============================================================================
...
BEGIN
    ...
    SELECT
        ...
        COALESCE(pq.utm_medium,   p.utm_medium)   AS utm_medium,
        COALESCE(pq.utm_campaign, p.utm_campaign) AS utm_campaign
...
-- Reload PostgREST schema cache so new column ordering is picked up
