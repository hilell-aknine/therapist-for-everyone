-- ============================================================================
-- Migration: admin_unified_source_funnel v2 — fix "registrations = leads" bug
--
-- Bug in v1 (20260430120000): "registrations" was computed as the subset of
-- leads (lead_attribution rows) who ALSO had a matching profile. This
-- structurally bounds registrations ≤ leads. But many real registrations
-- happen via Google OAuth or email signup with NO lead_attribution row in
-- between, so the true profile count is much higher than the leads count.
--
-- Hillel reported: dashboard showed 437 registered = 437 questionnaires,
-- but "many registered without filling [the questionnaire]". The 437=437
-- was an artifact: lead_attribution is heavily backfilled FROM
-- portal_questionnaires, so "leads" ≈ "questionnaires", and registrations
-- ≤ leads forced the equality.
--
-- v2 fix: registrations is now an INDEPENDENT count of profiles created in
-- the window. Source resolution per profile: attribution match > profile's
-- own utm_source column > 'unknown'. Same for paid.
--
-- The three stages — leads, registrations, paid — are now orthogonal counts
-- per source, not nested subsets.
-- ============================================================================
...
BEGIN
    ...
    WITH
    -- ── Population A: people who showed a lead-event (attribution row) in window
    attr_window AS (
    ...
    -- ── Population B: profiles created in window (registrations — independent of attribution)
    profile_window AS (
        SELECT
    ...
    -- ── Union of both populations (a person can be in both)
    union_window AS (
        SELECT
    ...
    -- ── Dedupe — same person from both populations becomes one row with both flags
    deduped AS (
        SELECT
            person_key,
    ...
    -- ── Lookup profile (any time) for people who weren't created in window but exist
    --    (so paid status works for older registrations who became leads recently)
    with_any_profile AS (
        SELECT
            d.*,
            COALESCE(
    ...
    -- ── Resolve paid status per person (active subscription on resolved profile)
    with_paid AS (
        SELECT
            wap.*,
            EXISTS (
    ...
    -- ── Bucket free-text how_found into canonical sources
    with_canonicals AS (
        SELECT
            wp.*,
            CASE
    ...
    -- ── Final source resolution: attribution UTM > profile UTM > how_found > unknown
    final AS (
        SELECT
            wc.*,
            COALESCE(
